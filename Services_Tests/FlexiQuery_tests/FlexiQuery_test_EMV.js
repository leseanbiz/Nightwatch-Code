...
var first_sheet_name = workbook.SheetNames[10];
...
module.exports = {
'FlexiQuery Test EMV Query' : function (browser) {
    ...
    for (z in worksheet) 
    {
        ...
        {
            emailsI[j] = worksheet[z].v;
            console.log(emailsI[j]);
        }
                ...
                else if (z.indexOf('F') != -1)
                {
                        ...
                        if(emailsI[j] != "blank")
                        {
                            browser.setValue('input[id="email"]',emailsI[j]);
                        }
                        
                        //Query is ran and waits for Results to be present before verifying text.
                        browser.useCss();    
                        browser.click('input[id="btnEmail2"]');
                        ...
                        if (riskLevelsR != "blank") 
                        {
                            browser.verify.containsText('table[id="resultsGrid"]',riskLevelsR[jR],"Age found in results for: " + jobCodesI[i]);    
                        }
                        if (agesR != "blank") 
                        {
                        ...
                        if (locationsR != "blank") 
                        {
                            browser.verify.containsText('table[id="resultsGrid"]',locationsR[lR],"Phone Number found in results for: " + jobCodesI[i]);
                        }
