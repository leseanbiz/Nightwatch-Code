...
module.exports = {
'FlexiQuery Test LIA Query' : function (browser) {
    ...
    for (z in worksheet) 
    {
        ...
        else if (z.indexOf('B') != -1)
        {
            ...
            //console.log(firstNamesI[j]);
                ...
                {
                        serviceTypesR[mR] = worksheet[z].v;
                        //console.log("Results last name is: " + lastNamesR[jR]);
                }
                        
                else if (z.indexOf('H') != -1)
                {
                        carriersR[nR] = worksheet[z].v;
                        //console.log("Results business name is: " + busNamesR[kR]);
                } 
                else if (z.indexOf('I') != -1)
                {
                        portedR[oR] = worksheet[z].v;
                        ...
                        if (ocnsR != "blank") 
                        {
                            browser.verify.containsText('table[id="resultsGrid"]',ocnsR[iR],"Name found in results for: " + jobCodesI[i]);    
                        }
                        if (locationsR != "blank") 
                        {
                            browser.verify.containsText('table[id="resultsGrid"]',locationsR[jR],"Verification found in results for: " + jobCodesI[i]);    
                        }
                        if (zipsR != "blank") 
                        {
                            browser.verify.containsText('table[id="resultsGrid"]',zipsR[kR],"Age found in results for: " + jobCodesI[i]);    
                        }
                        if (phonesR != "blank") 
                        {
                            browser.verify.containsText('table[id="resultsGrid"]',phonesR[lR],"Phone Number found in results for: " + jobCodesI[i]);
                        }
                        if (serviceTypesR != "blank") 
                        {
                            browser.verify.containsText('table[id="resultsGrid"]',serviceTypesR[jR],"Verification found in results for: " + jobCodesI[i]);    
                        }
                        if (carriersR != "blank") 
                        {
                            browser.verify.containsText('table[id="resultsGrid"]',carriersR[kR],"Age found in results for: " + jobCodesI[i]);    
                        }
                        if (portedR != "blank") 
                        {
                            browser.verify.containsText('table[id="resultsGrid"]',portedR[lR],"Phone Number found in results for: " + jobCodesI[i]);
                        }
